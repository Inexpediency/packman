...
function initWalls()
{
    ...
    for (var y = 0; y < FIELD.length; ++y)
    {
        for (var x = 0; x < FIELD[y].length; ++x)
        {
            if (FIELD[y][x])
            {
                walls[walls.length] = new Wall(x * FIELD_ELEMENT_SIZE, y * FIELD_ELEMENT_SIZE);
            }
        }
    }
    return walls;
...
{
    var gameState = {};
    gameState.packman = new Packman(1 * FIELD_ELEMENT_SIZE, 1 * FIELD_ELEMENT_SIZE, PACKMAN_CONST);
    gameState.walls = initWalls();
    return gameState;
}
